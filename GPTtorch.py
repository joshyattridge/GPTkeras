...
def generate_layers(
    ...
    *,
    ...
    base_user_content = (
        ...
        f"{feedback_context}"
        "Context: tabular inputs with 28 normalised features; binary target optimised with"
        " `torch.nn.CrossEntropyLoss` over two logits. Classes may be imbalanced up to 70/30, so incorporate"
        ...
        " Generate a PyTorch `nn.Sequential` for inputs of {cfg.input_dim} features and exactly {cfg.num_classes} logits;"
        " ensure the final layer is `nn.Linear(..., {cfg.num_classes})`. Hidden widths may draw from"
